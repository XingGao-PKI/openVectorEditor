...
const Axis = function (props) {
  ...
  const tickMarkPositions = useMemo(() => {
    ...
    return calculateTickMarkPositionsForGivenRange({
      ...
      const xCenter =
        (tickMarkPosition - (isProtein ? 1 : 0) + gaps) * charWidth + charWidth / 2;
      return {
        tickMarkPosition,
        xCenter
      };
    });
  }, [noRows, tickSpacing, row.start, row.end, sequenceLength, isProtein, charWidth]);
  ...
  tickMarkPositions.forEach(function ({ tickMarkPosition, xCenter }, i) {
    ...
    if (showAxisNumbers) {
      const position =
        normalizePositionByRangeLength(row.start + tickMarkPosition, sequenceLength) +
        (isProtein ? 0 : 1);
  ...
  return (
    ...
    >
      {tickMarkSVG}
      <path d={`M${xStart},${yStart} L${xEnd},${yStart}`} stroke="black" />
    </svg>
  );
};
