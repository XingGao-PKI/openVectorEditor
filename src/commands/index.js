...
const getNewTranslationHandler = isReverse => ({
  handler: (props, state, ctxInfo) => {
    const annotation = get(ctxInfo, 'context.annotation') || props.selectionLayer;
...
const fileCommandDefs = {
  ...
  duplicateSequence: {
    ...
    handler: (props, ...rest) => props.onDuplicate(props.sequenceData, props, ...rest),
    hotkey: 'alt+shift+d'
  },
  ...
  featureTypesCmd: {
    name: props => {
      ...
      const toHideCount = Object.keys(props.annotationVisibility.featureTypesToHide)
        .length;
      return (
        <span>
          Filter By Type &nbsp;
    ...
    submenu: props => {
      ...
      forEach(props.sequenceData.features, feat => {
        if (!feat.type) return;
        const checked = !props.annotationVisibility.featureTypesToHide[feat.type];
        if (types[feat.type]) {
  ...
  autoAnnotateHolder: {
    isHidden: props => !some(partsPrimersFeatures, type => props[`autoAnnotate${type}`])
  },
  ...partsPrimersFeatures.reduce((acc, type) => {
...
const noSelection = ({ selectionLayer = {} }) =>
  !(selectionLayer.start > -1 && selectionLayer.end > -1) && 'Selection Required';
...
const editCommandDefs = {
  ...
  createNewFromSubsequence: {
    ...
    isDisabled: props => props.sequenceLength === 0 || props.selectionLayer.start === -1,
    ...
    handler: props => {
      ...
      showDialog({
        ...
        props: {
          ...
          initialValues: {
            from: Math.max(1, 1 + divideBy3(start >= 0 ? start : 0, isProtein(props))),
        ...
        if (newSeq !== orginalSeq) {
          ...
          props.updateSequenceData({
            ...
            sequence: isSelection
              ? adjustBpsToReplaceOrInsert(sequence, newSeq, selectionLayer, false)
              : newSeq
          });
        }
      }
  ...
  newPart: {
    ...
    isHidden: props =>
      props.readOnly || !props.annotationsToSupport || !props.annotationsToSupport.parts,
...
const editAnnotationCommandDefs = ['feature', 'part', 'primer'].reduce((acc, key) => {
  acc[`edit${upperFirst(key)}`] = {
    name: props =>
      props.readOnly ? `View ${upperFirst(key)} Details` : `Edit ${upperFirst(key)}`,
    handler: (props, state, ctxInfo) => {
...
const deleteAnnotationCommandDefs = ['feature', 'part', 'primer', 'translation'].reduce(
  (acc, key) => {
...
[
  ...
  },
  {
    ...
    name: props => (isProtein(props) ? 'DNA Reverse Sequence' : 'Reverse Sequence')
  },
  {
    type: 'dnaColors',
    isDisabled: props =>
  ...
  annotationToggleCommandDefs[cmdId] = {
    ...
    name: props => {
      ...
      if (type === 'orfTranslations') {
        hasCount = true;
        count = filter(props.orfs || sequenceData.orfs || [], ({ isOrf }) => isOrf)
          .length;
      }
      return (
        <span>
