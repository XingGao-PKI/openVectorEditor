...
import {
  ...
  editorDragStarted,
  editorDragStopped,
  updateSelectionOrCaret as updateSelectionOrCaretUtil
...
function VectorInteractionHOC(Component /* options */) {
  return class VectorInteractionWrapper extends React.Component {
    ...
    componentDidMount() {
      ...
      this.combokeys.bind(getAcceptedChars(this.props.sequenceData).split(''), event => {
    ...
    handleCutOrCopy = isCut => e => {
      const { onCopy = () => {}, sequenceData, selectionLayer, copyOptions } = this.props;
      ...
      if (isCut) {
        ...
        onCut(e, tidyUpSequenceData(seqData, { annotationsAsObjects: true }), this.props);
      ...
      const makeTextCopyable = (transformFunc, className, action = 'copy') => {
        return new Clipboard(`.${className}`, {
          ...
          text: () => {
            ...
            const sequenceDataToCopy = transformFunc(selectedSeqData, sequenceData);
            ...
            ]),
        {
          ...
          submenu: [
            ...
            },
            {
              ...
              willUnmount: () => {
                this.openVeCopyComplement && this.openVeCopyComplement.destroy();
              },
              didMount: ({ className }) => {
    ...
    deletionLayerRightClicked = this.enhanceRightClickAction(({ annotation }) => {
    ...
    render() {
      ...
      // if (fitHeight) {
      // }
      const selectedBps = getSequenceWithinRange(selectionLayer, sequenceData.sequence);
...
function getGenbankFromSelection(selectedSeqData, sequenceData) {
  const spansEntireSeq = sequenceData.sequence.length === selectedSeqData.sequence.length;
